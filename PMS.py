...
conn = sqlite3.connect('D:\HTML-CSS COURSE\PMS.db')
...
conn = sqlite3.connect('D:\HTML-CSS COURSE\PPMS.db')
conn.execute("""CREATE TABLE IF NOT EXISTS PARKING (REG INT PRIMARY KEY NOT NULL,NAME TEXT NOT NULL,HOSTEL CHAR(5) NOT NULL,SEX CHAR(8) NOT NULL,MOBILE CHAR(12) NOT NULL,EMAIL CHAR(50) NOT NULL,
USERNAME CHAR(50) NOT NULL,PASSWORD CHAR(50) NOT NULL,VEHICLE CHAR(15) NOT NULL,PARK CHAR(10) NOT NULL, TYPE CHAR(12) NOT NULL ,STATUS CHAR(5) NOT NULL,SLOT CHAR(5) NOT NULL,PB CHAR(5) NOT NULL,C CHAR(5),CIN CHAR(50),COUT CHAR(50))""")
conn.commit()
conn.close()
...
def Base_Page():
    ...
    icon=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\LPU-Logo.png")
    logo=Label(root,image=icon)
    PMS=Label(root,text="PARKS\n(Parking Availability and Reservation Kiosk System)",font=("Helvetica", 32))
    logo.grid(row=1,column=0)
    PMS.grid(row=1,column=1,columnspan=2)
    
    icon1=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\Login.png")
    Log=Label(root,image=icon1)
    Login_Button=Button(root,text="Login",font=("Times New Roman",32),bd=3,command=Login_Page)

    icon2=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\Login.png")
    NUsr=Label(root,image=icon2)
    NewUser_Button=Button(root,text="New User",font=("Times New Roman",32),bd=3,command=BptoNu)
    icon3=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\Availability.png")
...
def Login_Page():
    ...
    PMS=Label(Login_GUI,text="PARKS\n(Parking Availability and Reservation Kiosk System)",font=("Helvetica", 32))
    logo.grid(row=1,column=0)
...
def validate():
    ...
    for row in cursor:
        if((str(name.get())=="Admin" ) and (str(pswd.get())=="Admin")):    # If Username and Password is of Admin, Login as Admin
            c=2
            break
        elif( (str(name.get())==row[0]) and (str(pswd.get())==row[1])):               # If Username & Password matches, Break & Proceed
            c=1
            break
...
def NewUser_Page():
    ...
    PMS=Label(NewUser_GUI,text="PARKS\n(Parking Availability and Reservation Kiosk System)",font=("Helvetica", 32))
    logo.grid(row=1,column=0)
...
def DB_Reg():
    ...
    if(str(Register.get())=="" or str(Name.get())=="" or str(Hostel.get())=="" or str(Mobile.get())=="" or str(Gender.get())=="" or str(Email.get())=="" or str(UserName.get())=="" or str(Pass.get())==""):
        ...
        conn = sqlite3.connect('D:\HTML-CSS COURSE\PMS.db')
        cott = sqlite3.connect('D:\HTML-CSS COURSE\PPMS.db')
...
def User_Page():
    ...
    PMS=Label(User_GUI,text="PARKS\n(Parking Availability and Reservation Kiosk System)",font=("Helvetica", 32))
    logo.grid(row=1,column=0)
    PMS.grid(row=1,column=1,columnspan=3)

    icon1=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\Book.png")
    ...
    icon2=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\Policy.png")
    ...
    icon3=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\Logout.png")
    Logout=Label(User_GUI,image=icon3)
    Logout_Button=Button(User_GUI,text="Logout",font=("Times New Roman",32),bd=3,command=UptoBp)

    icon4=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\Parking.png")
...
def Book_Page():
    ...
    global var
    var = StringVar()
    R1 = Radiobutton(Book_GUI, text="N Block Parking", variable=var, value="N block Parking",font=("Times New Roman",32))
    R2 = Radiobutton(Book_GUI, text="Main Gate Parking", variable=var, value="Main Gate Parking",font=("Times New Roman",32))
    global VehTyp
    VehTyp = StringVar()
...
# Empty Laber at Column 1
    ...
    checkIn_Button=Button(SuperUser_GUI,text="Check In",font=("Times New Roman",15),bd=3,width=12,command=CheckIN_page)
    checkIn_Button.grid(row=2,column=0)
    checkOUT_Button=Button(SuperUser_GUI,text="Check Out",font=("Times New Roman",15),bd=3,width=12,command=CheckOUT_page)
    checkOUT_Button.grid(row=2,column=1)
    
    icon4=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\Features.png")
...
def bennettLogo():
    ...
    icon=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\LPU-LogoDetails.png")
    logo=Label(SuperUser_GUI,image=icon)
    PMS=Label(SuperUser_GUI,text="PARKS\n(Parking Availability and Reservation Kiosk System)",font=("Helvetica", 48))
    logo.grid(row=1,column=1)
    PMS.grid(row=1,column=2,columnspan=4)

def ParkingDetails():

    icon1=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\ParkingDetails.png")
...
def CheckIN_page():
    ...
    icon2=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\CheckIN.png")
    ...
    global pd
    pd = StringVar()
    R1 = Radiobutton(SuperUser_GUI, text="N Block Parking", variable=pd, value="N Block Parking",font=("Times New Roman",17))
    R2 = Radiobutton(SuperUser_GUI, text="Main Gate Parking", variable=pd, value="Main Gate Parking",font=("Times New Roman",17))
...
def CheckOUT_page():
    ...
    icon3=PhotoImage(file="D:\HTML-CSS COURSE\PARKING MANAGEMENT SYSTEM\CheckOUT.png")
...
def AvailSlots():
    ...
    for row in cursor:
        if(row[0]=="Main Gate Parking" and row[1]=="Parked" and row[2]!="NIL"):
            S34[int(row[2])]=-1
        if(row[0]=="N Block Parking" and row[1]=="Parked" and row[2]!="NIL"):
            S29[int(row[2])]=-1
    for i in S34:
        if(i!=-1 and i!=0):
            A34=A34+str(i)+" "
